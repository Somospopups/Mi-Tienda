...
## v0.15.0 · 2026-09-26 · Complementos con foto, fondo y animaciones
- **Animaciones en todos los complementos**: cada bloque entra al aparecer en pantalla (sube, aparece, entra desde un lado, hace un acercamiento, entra con foco o gira en 3D) y sus tarjetas/fotos entran **escalonadas** una detrás de otra. Además, al pasar el mouse los botones, las tarjetas de testimonio, las fotos de la galería y las de producto se levantan con sombra suave. En el modo edición **no se anima nada**: el dueño ve el bloque exactamente como va a quedar. Si la persona tiene "reducir movimiento" activado en el sistema, la página aparece sin animaciones y sin dejar nada invisible.
- **Todo bloque acepta fondo con color o con imagen**: nuevo grupo **"Fondo y animación"** en la ficha de cada bloque, con **color de fondo**, **imagen de fondo**, **oscurecer la imagen (0-100)** y **esquinas redondeadas (0-40 px)**. Si ponés una foto y no tocás el oscurecido, se aplica un velo del 30% para que el texto se lea; con 0 no queda velo. El fondo también se puede **quitar** con un botón, y el radio en 0 deja las esquinas rectas.
- **Fotos propias en los complementos**: **Oferta** acepta una foto del producto (el bloque pasa a dos columnas: foto + texto, y se apila en el celu), y **Testimonios** y **Productos destacados** aceptan una **foto de cabecera**. La galería y el bloque de imagen siguen como estaban.
- **Guardado más seguro**: los fondos y las fotos de cabecera se comprimen más livianos (son decorativos) y, antes de guardar, si la página pasa el tope de 3 MB del contenido, se achican las fotos más grandes en vez de guardar el JSON cortado — antes eso dejaba la página vacía al siguiente guardado. Si aun así no entra, se avisa y **no se pisa** lo que ya estaba guardado.
- Tests: 8 nuevos en [`tests/page-builder-design.spec.js`](tests/page-builder-design.spec.js) (fondo de color/imagen y velo, velo por defecto, oferta con foto en dos columnas, fotos de cabecera, animación elegida y revelado al hacer scroll, "reducir movimiento", el tope de tamaño y el editor con los campos nuevos). El test de geometría ahora mide con "reducir movimiento" para que no dependa de las transiciones. Suite completa: **64/64**.
...
- **Complementos incluidos**: **Portada** (fondo, tono, alto, oscurecido, imagen y botón), **Oferta destacada** (badge "Hasta X% OFF", título, nota y botón al catálogo), **Testimonios** (hasta 6, en grilla de 3, con avatar de inicial), **Video/Reel**, **Productos destacados** (4, 8 o 12), **Banner con botón**, más los bloques base **Texto, Imagen, Galería, Reel de Instagram y Separador**.
- **Editor por bloque**: al tocar un bloque se abre su ficha a la izquierda del panel, con los campos que corresponden a su tipo (texto, link, número, área, color, imagen o carga múltiple). Las fotos se comprimen antes de guardar; los testimonios se escriben como `Nombre | Rol | Texto` (uno por línea). Botones por bloque: **editar, subir, bajar, duplicar y eliminar**.
- **Guardado**: los bloques van en `content.builderBlocks` (la clave que ya existía y respetaba el tope de 3 MB); en la nube viaja por `api_save_cfg` como el resto del contenido.
- **3 bugs corregidos de paso**: (1) el detalle de producto dentro de la página no abría — quedó handler solo en `#productGrid`; (2) tocar una tarjeta en modo edición **agregaba productos al carrito** (guard en `globalClick`/`catalogClick`); (3) la `×` del editor era inclicable porque la barra de edición (z-index 1080) tapaba el header del cajón — los cajones ahora arrancan debajo de la barra.
- Tests: 6 nuevos en [`tests/page-builder.spec.js`](tests/page-builder.spec.js) (panel y complementos, home reemplazado + catálogo a demanda, edición de propiedades, reordenar/duplicar/eliminar, compra pública desde la página, geometría de panel/editor) y 1 en [`tests/edit-mode-cloud.spec.js`](tests/edit-mode-cloud.spec.js) (el bloque se sincroniza con `api_save_cfg`). Suite completa: **50/50**.
